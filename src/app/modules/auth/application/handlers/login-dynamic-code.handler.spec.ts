...
describe('LoginDynamicCodeCommandHandler', () => {
  ...
  it('should successfully login dynamic code and return sessionToken', async () => {
    const userId = uuidv4();
    ...
    const mockUser = new User(userId, username, '1234567890', 'hashedpassword');

    mockJwtService.verify.mockResolvedValue({ sub: userId, purpose: 'dynamic_code_verification' });
    ...
    expect(mockJwtService.verify).toHaveBeenCalledWith(preAuthToken);
    expect(mockDynamicCodeService.verifyDynamicCode).toHaveBeenCalledWith(userId, dynamicCode);
    expect(mockUserRepository.findById).toHaveBeenCalledWith(userId);
    expect(mockJwtService.sign).toHaveBeenCalledWith({ sub: userId, username: username });
    expect(result).toEqual({ sessionToken: sessionToken });
  });
  ...
  it('should throw UnauthorizedException if preAuthToken is invalid', async () => {
    ...
    await expect(handler.execute(command)).rejects.toThrow('Invalid or expired pre-Authentication token.');
  ...
  it('should throw UnauthorizedException if preAuthToken purpose is incorrect', async () => {
    mockJwtService.verify.mockResolvedValue({ sub: uuidv4(), purpose: 'some_other_purpose' });
  ...
  it('should throw InvalidDynamicCodeError if dynamic code verification fails', async () => {
    ...
    await expect(handler.execute(command)).rejects.toThrow(InvalidDynamicCodeError);
    await expect(handler.execute(command)).rejects.toThrow('Invalid or expired dynamic code.');
  ...
  it('should throw UserNotFoundError if user not found after valid code', async () => {
    ...
    await expect(handler.execute(command)).rejects.toThrow('User not found after successful dynamic code verification.');
