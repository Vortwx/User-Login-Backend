...
describe('CreateUserCommandHandler', () => {
  ...
  it('should successfully create a new user', async () => {
    ...
    expect(mockUserRepository.save.mock.calls[0][0].username).toBe(username);
    expect(mockUserRepository.save.mock.calls[0][0].phoneNumber).toBe(phoneNumber);
    expect(mockUserRepository.save.mock.calls[0][0].hashedPassword).toBe(hashedPassword);
  ...
  it('should throw DuplicateUserError if username already exists', async () => {
    ...
    const password = 'Password123!';
    const phoneNumber = '1234567890';
    const existingUser = new User(uuidv4(), username, '0987654321', 'somehash');
    ...
    await expect(handler.execute(command)).rejects.toThrow(DuplicateUserError);
  ...
  it('should throw DuplicateUserError if phone number already createed', async () => {
    ...
    const password = 'Password123!';
    const phoneNumber = '1234567890';
    const existingUser = new User(uuidv4(), 'otheruser', phoneNumber, 'somehash');
    ...
    await expect(handler.execute(command)).rejects.toThrow(DuplicateUserError);
    await expect(handler.execute(command)).rejects.toThrow('Phone number already registered.');
    expect(mockHasherService.hash).not.toHaveBeenCalled();
    expect(mockUserRepository.save).not.toHaveBeenCalled();
  });
  ...
  it('should throw an error if username is empty (DTO validation)', async () => {
    ...
    await expect(handler.execute(command)).rejects.toThrow('Username cannot be empty.');
  });

  it('should throw an error if phone number is invalid (DTO validation)', async () => {
    const command = new CreateUserCommand('testuser', 'Password123!', '123');
    await expect(handler.execute(command)).rejects.toThrow('Invalid phone number format. Must be at least 10 digits.');
  });
});