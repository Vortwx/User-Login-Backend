...
describe('LoginUserCommandHandler', () => {
  ...
  it('should successfully login and return a preAuthToken', async () => {
    ...
    expect(mockHasherService.compare).toHaveBeenCalledWith(password, hashedPassword);
    ...
    expect(mockDynamicCodeService.storeDynamicCode).toHaveBeenCalledWith(userId, hashedDynamicCode, 5);
    expect(mockJwtService.sign).toHaveBeenCalledWith({ sub: userId, purpose: 'dynamic_code_verification' });
  ...
  it('should throw InvalidCredentialsError if user not found', async () => {
    ...
    await expect(handler.execute(command)).rejects.toThrow('Invalid username.');
    expect(mockHasherService.compare).not.toHaveBeenCalled();
    expect(mockDynamicCodeService.generateDynamicCode).not.toHaveBeenCalled();
  ...
  it('should throw InvalidCredentialsError if password does not match', async () => {
    ...
    await expect(handler.execute(command)).rejects.toThrow(InvalidCredentialsError);
    await expect(handler.execute(command)).rejects.toThrow('Invalid password for current username.');
    expect(mockHasherService.compare).toHaveBeenCalledWith(password, hashedPassword);
    expect(mockDynamicCodeService.generateDynamicCode).not.toHaveBeenCalled();
    expect(mockJwtService.sign).not.toHaveBeenCalled();
  });
