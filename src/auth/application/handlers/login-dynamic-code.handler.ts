...
import {
    ...
    LoginDynamicCodeCommandResponse,
} from '../commands/login-dynamic-code.command';
import {
    IUserRepository,
...
export class LoginDynamicCodeCommandHandler
    implements ICommandHandler<LoginDynamicCodeCommand>
{
    constructor(
    ...
    async execute(
        command: LoginDynamicCodeCommand,
    ): Promise<LoginDynamicCodeCommandResponse> {
        let userId: string;
        try {
            const payload: { sub: string; purpose: string } = await this.jwtService.verify(command.preAuthToken, process.env.JWT_TEMP_SECRET);
        ...
        const isCodeValid = await this.dynamicCodeService.verifyDynamicCode(
            userId,
            command.dynamicCode
        );
        if (!isCodeValid) {
